...
window.addEventListener('load', function () {
    ...
    ul.addEventListener('transitionend', function () {
        ...
        ol.children[index].classList.add('current'); // ！！！记得一定要加classList
    });
    ...
    ul.addEventListener('touchmove', function (e) {
        ...
        ul.style.transition = 'none';
        ul.style.transform = 'translate(' + translatex + 'px)'; // 让盒子动起来
    ...
    ul.addEventListener('touchend', function (e) {
        if (flag) {
            ...
            if (Math.abs(moveX) > 50) {
                ...
                var translatex = -index * w;
                ul.style.transition = 'all .3s'; // 加上过渡效果
                ul.style.transform = 'translate(' + translatex + 'px)';
            } else {
                // (2) 如果移动距离小于50像素 我们就播放上一张或者下一张
                var translatex = -index * w;
        ...
        // 手指离开的时候就重新开启定时器
        clearInterval(timer); // 开启之前先清除定时器，这样能保证当前这个页面里只有一个定时器
